...
import type { Category, Specialization } from "@/lib/types";
...
export default function SpecializationsPage() {
  ...
  const remove = async (id: string) => {
    ...
    await fetch(`/api/admin/specializations?id=${id}`, { method: "DELETE" });
    load();
  };
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-2xl font-bold text-primary">{t("admin.menu.specializations")}</h1>
        <Button onClick={() => setEditing({ nameAr: "", nameEn: "", categoryId: categories[0]?.id })}>
